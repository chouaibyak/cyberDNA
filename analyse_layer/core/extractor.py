    ...
    def extract_features(self, normalized_logs, window_seconds=300):
        ...
        for log in normalized_logs:
            if log and log["source_ip"]:
                # Calcul de l'ID de la fenêtre (ex: toutes les 60 secondes)
                ts = self._parse_timestamp(log["timestamp"])
                window_id = int(ts.timestamp() / window_seconds)
                
                # La clé est un tuple (IP, ID_Fenêtre)
                key = (log["source_ip"], window_id)
                windows[key].append(log)

        features_results = {}
        
        for (ip, win_id), logs in windows.items():
            # --- TOUTE TA LOGIQUE DE CALCUL RESTE EXACTEMENT LA MÊME ---
            ...
            if total_events > 1:
                ...
                eps = total_events / max(duration, 0.1)
                iat = duration / (total_events - 1)
            else:
                eps, iat = 0, 10

            # On crée une clé unique "IP_ID" pour l'IA
            unique_id = f"{ip}_{win_id}"
            features_results[unique_id] = {
                "ip": ip, # On garde l'IP pour savoir qui c'est
